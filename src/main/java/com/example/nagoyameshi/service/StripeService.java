...
public class StripeService {
	...
	// 支払い方法（StripeのPaymentMethodオブジェクト）を顧客（StripeのCustomerオブジェクト）に紐づけ
	...
	// 顧客（StripeのCustomerオブジェクト）のデフォルトの支払い方法（StripeのPaymentMethodオブジェクト）を取得
	...
	public void detachPaymentMethodFromCustomer(String peymentMethodId) throws StripeException {
		PaymentMethod paymentMethod = PaymentMethod.retrieve(peymentMethodId);
	...
	// サブスクリプション（StripeのSubscriptionオブジェクト）をキャンセル
