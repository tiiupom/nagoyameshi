...
public class ReviewController {
	...
	// レビュー一覧ページ（reviews/index.htmlファイル）を表示
	@GetMapping
	public String index(@PathVariable(name = "storeId", required = false) Integer storeId,
	...
	// レビュー投稿ページ（reviews/register.htmlファイル）を表示
	@GetMapping("/register")
	...
	// レビュー編集ページ（reviews/edit.htmlファイル）を表示す
						...
						RedirectAttributes redirectAttributes)
	{
		...
		if (user.getRole().getName().equals("ROLE_GENERAL")) {
			redirectAttributes.addFlashAttribute("subscriptionMessage", "この機能を利用するには利用プランへの登録が必要です。");
